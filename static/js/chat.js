document.addEventListener('DOMContentLoaded', () => {
    ...
    function addMessage(message) {
        ...
        // Add reply button and thread count if not a reply
        if (!message.parent_id) {
            messageFooter = `
                <div class="message-footer">
                    <button class="btn btn-link btn-sm reply-button" data-message-id="${message.id}">
                        ...
                        ${message.thread_count > 0 ? `<span class="thread-count">${message.thread_count}</span>` : ''}
                    </button>
                </div>
            `;
        }
